...
| `full-failure`    | Valid preview; invalid full image | failure status, named preview fallback, Retry, and usable close             |
| `preview-failure` | Invalid preview; no full image    | preview failure status and no invented full-image Retry                     |

The invalid sources are self-contained malformed image data URLs. They do not depend on a network
intercept or a missing server path.

### Non-live event trace

The event trace records `open-requested`, `opened`, `indexChanged`, `requestClose`, `update:open`,
and `closed`, including index and reason where the public event supplies them. It has
...
7. Press Escape. Verify the dialog closes, focus returns to the exact opener, and the trace ends
   with `requestClose` reason `escape`, `update:open false`, then `closed` (C10).
8. Run `single-item`. Read both directional controls and record how unavailability is conveyed.
   Verify one named image and no background escape (C03-C05).
9. Run `full-failure`. Wait for the failure state. Record exact speech for the failure, preview, and
   Retry. Activate Retry once, then close. Verify C08; a repeated failure is acceptable only if it
   remains understandable and operable.
10. Run `preview-failure`. Record the failure and confirm no Retry is offered (C09).
...
6. Repeat `single-item`, `full-failure`, and `preview-failure` using steps 8-10 from the Firefox
   script. Record any difference from Firefox rather than copying its result.
...
7. Run `single-item`, `full-failure`, and `preview-failure`. Record rotor order, exact failure
   speech, Retry operability, image alt text, and C08-C09 results.
...
7. Run `single-item`, `full-failure`, and `preview-failure`; record C04-C05 and C08-C09. After the
   full failure, activate Retry once and confirm Close remains reachable.
...
7. Run `single-item`, `full-failure`, and `preview-failure`. Record exact failure output, the named
   fallback preview, Retry operability, and C08-C09.
8. Repeat one baseline navigation with Motion `Reduced`.
